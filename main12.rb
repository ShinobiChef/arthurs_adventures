...
module Tiles
 ...
 class Player
  ...
  def aim_down
    ...
    def shootleft
    #@window.objects << Dagger.new(@window, $x + 10 * @dir, $y - 10, @angle * @dir)
    @window.objects << Dagger.new(@window, $scrx +25 , $scry-20, @angle * -1)
    
  end
  
  def shootright
    #@window.objects << Dagger.new(@window, $x + 10 * @dir, $y - 10, @angle * @dir)
    if $gamemode ==2 then @window.objects << Dagger.new(@window, $scrx -55, $scry-20, @angle * +1) end
    if $gamemode == 20 then @window.objects << Dagger.new(@window, 45, 730, @angle * +1) end
  end
  
  def shootrightdagger
    #@window.objects << Dagger.new(@window, $x + 10 * @dir, $y - 10, @angle * @dir)
    @window.objects << Dagger.new(@window, 100, 300, @angle * +1)
...
# Implements the same interface as Player, except it'S a dagger!

class Dagger
  attr_reader :x, :y, :vx, :vy
  
  def initialize(window, x, y, angle)
    # All dagger instances use the same sound.
  ...
  def hit_by?(dagger)
    # Daggers can't be hit by other daggers!
    false
  end
end
...
class Enemyspr
  ...
  def aim_down
    ...
    def shootleft
    #@window.objects << Dagger.new(@window, $x + 10 * @dir, $y - 10, @angle * @dir)
    @window.enemyattackobjects << Enemyshoot.new(@window, 1000,730, @angle * -1)
...
class Game < Window
  ...
  :scrx,:scry,:scry1,:isshooting,:dx,:dy,:enemyhp,:enemymaxhp,:enemytype,:eax,:eay,:enemyattackobjects,:rannumgen
  ...
  def initialize
    ...
    @players = [Player.new(self, 200, 40, 0xff308000)]
        @objects = @players.dup
    @enemyplayers = [Enemyspr.new(self, 1000, 700, 0xff308000)]
        @enemyattackobjects = @enemyplayers.dup    
        
    self.caption = "Arthur's Adventures V0.5"
...
$dir = :right
#if $dx>1000 then $hitsound.play and @objects.reject! { |o| o.update} and $dx = 0  end


@objects.grep(Dagger).empty?
@enemyattackobjects.grep(Enemyshoot).empty?

@objects.reject! { |o| o.update == false }
     
     @enemyattackobjects.reject! { |o| o.update == false }
...
#if $x-@screen_x>$dx-25 and $x-@screen_x<$dx+25 and $y-@screen_y>$dy-25 and $y-@screen_y<$dy+25 then $hitsound.play end
...
@objects.reject! { |o| o.update == false }
    ...
    move_x -= 5 if button_down? Button::GpLeft
    move_x += 5 if button_down? Button::KbRight
    move_x += 5  if button_down? Button::GpRight
    
    if move_x !=0  and @rannumgen>0 then @rannumgen -=1 end
    if @rannumgen == 0 then  @rannum = (rand 250)+1 end
    
     if @rannum == 250 then @menumovesound.play and  setupenemy end
...
if $gamemode == 1 then
  ...
  @titlescreenimage.draw(0, 0, 0)
    ...
    @fontsmall.draw("V0.5" , 1230, @gameyres -20,1, 1.0, 1.0, 0xffffffff)
...
$normalareatileset = Image.load_tiles(self, "data/tiles/normalrealmtileset.png", 60, 60, true)
 ...
 @currentareabkimage = Image.new(self, "data/backgrounds/blank.png", true)
    ...
    @currentfightbkimage = Image.new(self, "data/backgrounds/fightarea2bk.png", true)
...
@sprhit = Image.new(self, "data/items/hitsprite.png", true)   

@sprsnake = Image.new(self, "data/enemys/snaker.png", true)   
...
if $currentarea == 15 and $x>12475 and $x<12495 and $y<9950 and $y>9800 then 
  ...
  if $currentarea == 9 then @currentareabkimage = @area9bk and @map = $area9map end
    ...
    @font.draw("Exp: #{$arthurexp}" , 1070, 30,1, 1.0, 1.0, 0xffffffff)
    ...
    @font.draw("dX:#{$dx} dY:#{$dy}" , 700, 170,1, 1.0, 1.0, 0xffffffff)
...
draw_line(10,100,@hpbarcolor,$arthurhp+10,100,@hpbarcolor,z=0)
draw_line(10,101,@hpbarcolor,$arthurhp+10,101,@hpbarcolor,z=0)
draw_line(10,102,@hpbarcolor,$arthurhp+10,102,@hpbarcolor,z=0)
draw_line(10,103,@hpbarcolor,$arthurhp+10,103,@hpbarcolor,z=0)
draw_line(10,104,@hpbarcolor,$arthurhp+10,104,@hpbarcolor,z=0)
draw_line(10,105,@hpbarcolor,$arthurhp+10,105,@hpbarcolor,z=0)
...
if $gamemode == 20 then
    ...
    @currentfightbkimage.draw(0,0, 0)
    ...
    @objects.each { |o| o.draw }
    @enemyattackobjects.each { |o| o.draw }
    #if $dx>1000 then $hitsound.play and @objects.reject! { |o| o.update} and $dx = 0 and @sprhit.draw($dx,$dy, 0)  end
    
    if $enemyhp>0 then 
      @sprsnake.draw(1000,700, 0)
      end
    
    if $dx>1000 then 
      @sprhit.draw($dx,$dy, 0)
     $hitsound.play
      @objects.reject! { |o| o.update} 
      $dx = 0 
     $enemyhp -= 15
     if $enemyhp >0 then enemyattack end
     if $enemyhp <0 then $enemyhp = 0  and $gamemode = 2  and enemydead end
      end
    
    if $eax>0 and $eax<50 then 
      @sprhit.draw($eax,$eay, 0)
     $hitsound.play
      @enemyattackobjects.reject! { |o| o.update} 
      $eax = 0 
     if $enemytype == 1 then $arthurhp -= 10 end
     
     if $arthurhp <0 then $arthurlives = 0  end
      end
    ...
    @font.draw("scrX:#{$scrx} scrY:#{$scry}" , 700, 150,1, 1.0, 1.0, 0xffffffff)
    @font.draw("dX:#{$dx} dY:#{$dy}" , 700, 170,1, 1.0, 1.0, 0xffffffff)
    @font.draw("eaX:#{$eax} eaY:#{$ea}" , 700, 190,1, 1.0, 1.0, 0xffffffff)
    
    
    @fontsmall.draw("Current area:#{$currentarea} selectedgroup:#{$selectedgroup} " , 1000, 100,1, 1.0, 1.0, 0xffffffff)
    @fontsmall.draw("Enemy HP:#{$enemyhp}/#{$enemymaxhp} " , 1000, 150,1, 1.0, 1.0, 0xffffffff)
    #@fontsmall.draw("rannum:#{@rannum} " , 1000, 130,1, 1.0, 1.0, 0xffffffff) 
...
if $gamemode == 20 then 
          ...
          if id == Gosu::KbD or id == Button::GpButton1  and $dx ==0 then
...
if $gamemode == 2 then 
           ...
           if id == Gosu::KbT then $gamemode = 20  and $enemyhp = 50  and $enemymaxhp = 50 end
