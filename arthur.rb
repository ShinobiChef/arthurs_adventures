...
class Arthur
  ...
  def initialize(window, x, y)
    ...
    @angle = 90
        ...
        @standing, @walk1, @walk2, @jump, @wall_climb ,@shoot1 ,@shoot2=
   ...
   def draw(screen_x, screen_y)
        ...
        if $gamemode == 2 then @cur_image.draw($x - screen_x  + offs_x, $y - screen_y - 58, 0, factor, 1.2) end
        if $gamemode == 20 then @cur_image.draw(100,700, 0, factor, 1.2) end
        
 $scrx= $x - screen_x  + offs_x
 $scry1 = screen_y   
 ...
 if $isshooting > 5 then  @cur_image = @shoot1 end
 
 if $isshooting < 5 and $isshooting > 0 then  @cur_image = @shoot2 end
if $isshooting > 0 then  $isshooting -= 1 end
# Acceleration/gravity
   ...
   def setnewgamevars
    ...
    @arthurstatus1, @arthurstatus2, @arthurstatus3, @arthurstatus4, @arthurstatus5, @arthurstatus6 = 0,0,0,0,0,0
      ...
      $arthurexp, @arthurlevel, $arthurlives = 0,1,1
